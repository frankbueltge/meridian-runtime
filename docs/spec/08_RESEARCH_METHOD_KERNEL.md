...
| `MethodProtocol` | the lockable commitment document of a profile (a synthesis protocol, a pre-analysis plan, …): criteria, rules, analyses, kill conditions | draft → reviewed → **locked** → amended \| executed; amended → reviewed (re-review cycle: an amendment is a new revision that must be re-reviewed and re-locked before further confirmatory work; each lock binds that revision's own content hash, so work recorded under an earlier lock stays auditable against its own hash) |
