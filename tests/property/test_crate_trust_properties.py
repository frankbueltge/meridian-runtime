"""Property test: ``mrr.domain.crate_trust.resolve_trusted_crate_key`` never
returns a key for any of its four failing preconditions (task-packets/
E5-T05.yaml acceptance test: "fail-closed matrix, each a distinct typed
error" — proved here at the resolver level: the resolver never returns a
...
the seven concrete fault kinds is injected into an otherwise fully
...
Local, deliberate duplicate of tests/unit/domain/test_crate_trust.py's own
...
#: The seven concrete ways ``resolve_trusted_crate_key`` must fail closed —
...
def _practice(*, practice_id: str, keys: list[dict[str, Any]]) -> Practice:
    data: dict[str, Any] = {
        ...
        "description": "Fixture practice for crate trust property tests.",
        "keys": keys,
...
def _crate(*, signer_practice_id: str, key_id: str) -> EvidenceCrate:
    now = _NOW
    data: dict[str, Any] = {
...
def test_resolver_never_returns_a_key_for_a_failing_precondition(fault: str) -> None:
    ...
    signed = _sign(crate, private_key)
    ...
    if fault == "unknown_kid":
        ...
        expected_error = CrateKeyNotValidError
    elif fault == "signer_mismatch":
        trusted_practice_id = new_urn("practice")
        expected_error = CrateSignerMismatchError
    elif fault == "tampered":
        signed = signed.model_copy(update={"run_state": "failed"})
    ...
    with pytest.raises(expected_error):
        resolve_trusted_crate_key(signed, trusted_practice_id, ring, at=_NOW)
...
def test_resolver_returns_the_key_when_every_precondition_holds() -> None:
    ...
    resolved = resolve_trusted_crate_key(signed, practice_id, ring, at=_NOW)
