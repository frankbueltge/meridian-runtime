"""Unit tests for mrr.domain.transfer_trust (task-packets/E6-T01.yaml).

Covers the packet's named acceptance tests at the domain layer: a Practice
with an active in-window key trust-anchors and resolves a TransferContract it
...
error — mirrors the E5-T05 crate-verification matrix exactly, per the
packet's own acceptance test wording; a key valid at signing but revoked by
the evaluation instant is rejected while its descriptor remains resolvable
(docs/spec/04 section 8.4); and the key-substitution attack (an attacker key
claiming a trusted kid fails closed with SignatureVerificationError, because
...
fixture-building convention, adapted to TransferContract (this codebase's
...
def _practice(*, practice_id: str, keys: list[dict[str, Any]]) -> Practice:
    data: dict[str, Any] = {
        ...
        "description": "Fixture practice for transfer trust unit tests.",
        "keys": keys,
...
def _contract(
    ...
    **overrides: Any,
    ...
    data: dict[str, Any] = {
        ...
        "practice_id": sender_practice_id or signer_practice_id,
        "revision": 1,
        "created_at": now,
        "created_by": new_urn("agent-role"),
        "content_hash": "sha256:" + "a" * 64,
        "sender_practice_id": sender_practice_id or signer_practice_id,
...
def _sign(contract: TransferContract, private_key: Ed25519PrivateKey) -> TransferContract:
    ...
    canonical body ``resolve_trusted_transfer_key`` verifies against.
    """
...
def _trusted_scenario(
    *,
    ...
    valid_from: datetime = _VALID_FROM,
    valid_until: datetime = _VALID_UNTIL,
) -> tuple[TransferContract, Practice, Ed25519PrivateKey]:
    """A fully self-consistent scenario: a Practice with one key in
    ``key_state``, and a TransferContract genuinely signed by that key,
    naming the practice as sender/signer.
    """
    ...
    return _sign(contract, private_key), practice, private_key
...
def test_happy_path_resolves_the_trusted_verifying_key() -> None:
    contract, practice, private_key = _trusted_scenario()
    ring = practice_key_ring(practice)

    resolved = resolve_trusted_transfer_key(contract, practice.id, ring, at=_NOW)
...
def test_unknown_kid_raises_unknown_key_id_error() -> None:
    contract, practice, _ = _trusted_scenario()
    ring = practice_key_ring(practice)
    tampered = contract.model_copy(
        update={"signature": contract.signature.model_copy(update={"key_id": "kid:does-not-exist"})}
    )

    with pytest.raises(UnknownKeyIdError) as excinfo:
        resolve_trusted_transfer_key(tampered, practice.id, ring, at=_NOW)
    assert excinfo.value.kid == "kid:does-not-exist"


def test_revoked_key_raises_transfer_key_not_valid_error() -> None:
    contract, practice, _ = _trusted_scenario(key_state="revoked")
    ring = practice_key_ring(practice)

    with pytest.raises(TransferKeyNotValidError):
        resolve_trusted_transfer_key(contract, practice.id, ring, at=_NOW)


def test_rotated_key_raises_transfer_key_not_valid_error() -> None:
    contract, practice, _ = _trusted_scenario(key_state="rotated")
    ring = practice_key_ring(practice)
...
def test_expired_key_raises_transfer_key_not_valid_error() -> None:
    contract, practice, _ = _trusted_scenario(
...
def test_not_yet_valid_key_raises_transfer_key_not_valid_error() -> None:
    contract, practice, _ = _trusted_scenario(
...
def test_signer_mismatch_raises_transfer_signer_mismatch_error() -> None:
    contract, practice, _ = _trusted_scenario()
    ring = practice_key_ring(practice)
    other_practice_id = new_urn("practice")

    with pytest.raises(TransferSignerMismatchError) as excinfo:
        resolve_trusted_transfer_key(contract, other_practice_id, ring, at=_NOW)
    assert excinfo.value.claimed_signer_practice_id == contract.signature.signer_practice_id
    assert excinfo.value.trusted_practice_id == other_practice_id


def test_tampered_contract_raises_signature_verification_error() -> None:
    contract, practice, _ = _trusted_scenario()
    ring = practice_key_ring(practice)
    tampered = contract.model_copy(update={"purpose": "not what was signed"})
...
# key the contract itself claims.
# ---------------------------------------------------------------------------
...
def test_key_substitution_attack_fails_closed_with_signature_verification_error() -> None:
    ...
    resolve_trusted_transfer_key decodes and verifies against the RING's
    descriptor for that kid, never anything the contract itself carries, so
    ...
    contract = _contract(signer_practice_id=practice_id, key_id=entry["kid"])
    forged = _sign(contract, attacker_private_key)
    ring = practice_key_ring(practice)

    with pytest.raises(SignatureVerificationError):
        resolve_trusted_transfer_key(forged, practice_id, ring, at=_NOW)
...
def test_key_valid_at_signing_but_revoked_by_instant_is_rejected() -> None:
    """A contract signed while the key was active must still be rejected if
